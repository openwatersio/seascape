...
## Depth areas and contours compiled per zoom

An ENC never declutters its depth areas by hiding some of them: DEPARE is the skin of the earth, must tile the cell without gaps at every scale, and may not carry a display-scale minimum (S-57 forbids SCAMIN on Group 1; S-101 says the same of the skin). Scale is handled by compiling a separate, more generalized cell per navigational purpose, each with its own compilation scale and contour set. The tiles do the same in zoom tiers: each tier is its own gapless partition, cut from the surface at the tier's first zoom with the isobath ladder drawn there, and it serves every zoom up to the next tier; the last tier a stem holds is cut from its native window and serves everything above. Isobaths derive from the same tier's partition edges, so a tint boundary sits on a drawn contour at every zoom.

Zoomed out, the full isobath set is noise — abyssal contours stipple at small scales. Charts thin the deep, not the shelf: coarse tiers carry only the widely spaced deep levels, and each tier in adds finer levels until everything shows (the ladder per zoom band follows NOAA's ENC design table). Which levels appear at which zoom is a display decision the tiles make; it is deliberately _not_ part of the schema contract.

A tier's surface is the mosaic's own class-aware shoal pyramid level, smoothed at that zoom exactly as the raster render smooths it, so isobaths and shading agree; the smoothing is depth-gated, aggressive in the abyss and minimal in the 0–30 m band. Each tier's surface is then clamped shoal-ward against the reduction of the next finer tier, so the compiled zooms cascade from the native cut out and zooming out can only shoal (S-4 B-411.5: smoothing "must be shoal-biased"). Fragments that survive the coarse pixel but are still under 4 mm² at the tier's scale (Galanda's floor for a solid fill) are generalized the way Guilbert and Zhang state the rule for isobaths: a part with a shallower neighbour dissolves into the shallowest of them (a pit removed), a part with land or drying all round drops, and a part whose every neighbour is deeper is a shoal and stays.
