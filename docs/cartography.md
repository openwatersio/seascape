...
Zoomed out, the full isobath set is noise — abyssal contours stipple at small scales. Charts thin the deep, not the shelf: coarse tiers carry only the widely spaced deep levels, and each tier adds finer levels until everything shows (the ladder per zoom band follows NOAA's ENC design table). Which levels appear at which zoom is a display decision the tiles make; it is deliberately _not_ part of the schema contract.
