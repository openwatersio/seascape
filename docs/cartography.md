...
The raster's per-zoom vertical quantization (coarse steps at low zoom, full resolution deep in) was designed for terrain, where relief is measured in kilometres. Applied naively to a shelf it terraces the shallows: a 2 m step in 3 m of water is a cliff. So the step is additionally capped per-pixel at 1/16 of the local depth (floored at 0.25 m, matching sounding label precision in the shoal band). Deep water keeps the byte-saving coarse steps; shallow water keeps chart detail at every zoom, and the step never spans more than one colour band.
