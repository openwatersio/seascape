...
- **A bbox build cuts coarse depth-area tiers against a truncated neighbourhood.** A tier's surface reads the mosaic with the smooth halo at the tier's own resolution — tens of kilometres at z6 — so `depare_tile`'s input set is the union of every tier's read set, and the planet build after a bbox smoke re-forks every in-window stem whose widest halo left the window — depare and contours by the input-set trigger, soundings through the regenerated `temp()` window. One smoke stem is a handful of jobs; it is the correct heal, not churn.
