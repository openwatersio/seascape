...
1. **`read_bucket` single-scan refactor.** The wedge: FGB has no attribute index, so each per-level bucket read full-scans the raw partition and re-parses every feature's WKB — ~19 scans per ladder, 5,650 s on the Arctic stem whose giants cost ~300 s per scan. Read once per ladder, split by `amax` in memory, feed downstream unchanged. Diagnosis agent owes the measured prototype + equivalence run.
