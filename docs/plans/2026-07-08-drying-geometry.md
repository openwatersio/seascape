...
- **Boxiness — raster origin.** Drying is a binary threshold (`elev in [0, cap]`) rasterized to a Byte mask and traced with the GDAL polygonizer (`drying_run.py`, the raster path this PR removes), so every edge is a 90° staircase at the DEM pixel pitch. The only cleanup is a deliberately weak Douglas–Peucker that keeps axis-aligned runs straight by design. The depth bands and contour lines in the same layer come from `gdal_contour` ([depare_run.py](../../pipelines/depare_run.py), [contour_run.py](../../pipelines/contour_run.py)) — interpolated sub-pixel isolines — so they're smooth. The two diverge entirely at raster-threshold vs. interpolated-isoline.
...
The seaward edge of a drying area *is* the chart-datum (0 m) isobath — which `gdal_contour` already computes for the depth bands (`DEPARE_LEVELS` ends at 0, [config.py:43](../../pipelines/config.py)) and, before this PR, discarded as "land". So the smooth geometry we want was already being produced and thrown away; this plan reclaims it.
...
- **Cut the landward side by EFFECTIVE land, not raw OSM land.** This is the load-bearing correctness point. The drying region is `[0, cap]` restricted to *effective water* — exactly the pre-PR raster gate `in_range & (mask == 0)` (`drying_run.py`), where `mask` is the land∖water raster (`rasterize` burns land=1 then water=0, [landmask.py:243,249](../../pipelines/landmask.py)). So the vector cutter must be **`effective_land = OSM_land ∖ OSM_inland_water`**, not the raw land polygons.
...
- **Chaikin the drying ring.** Rejected in the prior raster code and here: self-intersects on thin foreshore necks, biases inward, and per-feature smoothing opens see-through cracks against the abutting band ([depare_run.py](../../pipelines/depare_run.py)).
...
- **`drying_run.py` fate — resolved in this PR.** `_polys` was the only survivor; it moved into `depare_run` and `drying_run.py` was deleted (nothing else consumed the module — the standalone per-tile drying FGB had no remaining reader once depare derives drying itself).
