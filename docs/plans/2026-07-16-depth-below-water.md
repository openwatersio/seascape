...
Accepted caveats: overzoom/smoothing interpolation still feathers lake edges between the tints (rendering samples, per the resampling caveat above), and drying foreshore `(0, cap]` keeps rendering land-ish in relief-only mode — depare supplies the drying tint.

**Preview finding (2026-07-21): the sentinel halo at inland water.** With land collapsed to a uniform 17 m plateau, every 0-filled OSM water polygon (all #24-cleared lakes/rivers, not just the big ones) meets the plateau in a single-pixel 0→17 step, which client hillshade renders as a crisp halo — before the clamp those edges met locally-varying terrain and read as landscape. No image encoding fixes this (raster-dem decodes every opaque pixel; alpha is premultiplied; a big fill makes a bigger cliff), so the step can only be softened or moved. Fix shipped: `FEATHER_PX` in `terrain.py` — plateau pixels ramp from 0 to the sentinel over 4 px of distance to the nearest ≤0 pixel, touching ONLY sentinel pixels (never water, never genuine low land; padded reads keep tile seams exact). Ramp values are `(0, cap]` transition samples the contract already tolerates. The complete fix, if the softened halo still bothers: a Worker-derived depth view (`min(v, 0)` decode→clamp→re-encode of the same archive, no second pipeline product) as the hillshade source, with color-relief staying on the full product. The nudge applies only to regional render windows (`cz >= 8`): a planet-scale window's `-spat` clip streams the entire multi-GB vector mask rather than an indexed subset (found the hard way — coarse stems hung then died mid-stream). At z≤7 land-side exact-0 areas are a few pixels and the dominant exact-0 areas (large unknown lakes) should tint water anyway; a rasterized planet-z8 mask artifact (mirroring `planet-z8.tif`) is the upgrade path if coarse-zoom nudging ever matters. Composes with the sentinel clamp (same code region, same render-only re-key); ship both in one PR, one planet re-render.
...
>
> - `v < 0` — elevation below the winning source's datum; on measured water pixels `-v` is the charted depth. The *encoding* is shallow-biased (quantize never deepens); the *datum* is per-source — GEBCO/EMODnet are ≈MSL, which sits above any low-water datum, so their depths read deep vs chart datum by the local MSL−LAT separation until datum unification.
...
A note that strengthens every flattening option: the PMTiles writer dedups identical tile bytes (hash → offset + run-length in `pmtiles.writer.Writer.write_tile`), so once inland tiles quantize to an identical flat plateau they are stored roughly once per zoom level (the sentinel quantizes to +18 at z≤10 and +17 at z≥11, so the flat tile's bytes differ across zooms) — inland coverage becomes directory entries, not payload.
...
2. **Fourth-quadrant mosaic fix** (positive-in-ocean → 0 for `land_clamp` sources): the sentinel hides coarse false land in the _raster_, but the _drying bucket and depare_ read the unclamped mosaic, so GEBCO shoreline cells in `(0,16]` seaward of the OSM line still fabricate foreshore without it. Correction (2026-07-21): this did NOT exist in the code despite the "keep" wording — implemented fresh as `landmask.clamp_positive_ocean`, a sibling of `clamp`/`clamp_positive_water`, keyed on the combined mask (not land) intersected with the water-only mask (not inland water) → ocean; positive ocean clamps to 0. Two review caveats (2026-07-21): this is the one deliberate deep-ward move in the design — +2 m "drying" → 0 charts that seabed lower, betting the true seabed under a coarse coastal cell is subtidal; and on remote macrotidal coasts where a flagged source is the SOLE coverage (Canadian Arctic, Patagonia, NW Australia), genuine coarse intertidal signal degrades to unknown-depth water — a bounded loss (both read non-safe-water), accepted because no chart would assert drying heights from a ~460 m cell.
