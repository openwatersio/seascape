...
2. **Local dependencies**: This is faster for iterative development but requires a lot of local setup, could be brittle, and is subject to change. You'll need:
   ...
   - **GDAL CLI** — `gdalwarp`, `gdal_translate`, `gdal_contour`, `ogr2ogr`, `gdalbuildvrt`, `ogrinfo`. Use a recent build (the container pins GDAL master, for the fast `gdal_contour -p` of OSGeo/gdal#14983, #15181 and #15182): 3.8-era polygon-contour mode mis-writes the deepest depth-area bucket's `amin`, and released GDAL through 3.13 is quadratic in ring count on marsh coastlines.
   - **tippecanoe** — vector tiles. Must include felt/tippecanoe#397 and #399 (per-feature `tippecanoe.minzoom` survives `--generate-variable-depth-tile-pyramid`, and leaf pruning respects pending minzooms); the Docker image pins the #399 merge commit. No released or brew tippecanoe has both fixes yet — an older binary silently drops minzoom-gated features under variable depth (a local `just preview` will then fail the vector self-check). Build from the pin, or run previews through `./docker.sh`.
...
- **aggregation** (`aggregation_*.py`): `cover` slices the planet into source-aware aggregation tiles (one CSV each); `run` reprojects each source by priority into a merged Float32 DEM (Gaussian seam feather), slope-smooths it (`smooth.py`), encodes Terrarium raster tiles, and forks soundings (`soundings_run.py`) and depth areas (`depare_run.py`) off the same merged DEM, with isobath lines (`contour_run.py`) derived from the depth-area edges — each fork is a separate Snakemake job, so e.g. a contour-levels change re-runs the vector forks without rewriting terrain pmtiles. Sources flagged `land_clamp` (GEBCO/EMODnet — coarse, no land/water concept) get their negative land pixels clamped to 0 against the OSM land mask (`landmask.py`, prep once with `just landmask`) right after warp, so shoreline cells don't paint false water/contours/soundings over land; mapped inland water reopens the clamp only where it is at least the source's own cell wide, so a polder ditch under GEBCO stays land and the depare nodata layer marks it unknown rather than charting the polder's elevation as depth. A `depare_run.py` fork buckets the same merged DEM into depth-area partitions (ENC DEPARE: water between charted isobaths, `drval1`/`drval2` depth bounds), one per zoom tier (`config.depare_tiers`: coarse tiers cut from the mosaic pyramid level at the tier's first zoom with the isobath ladder drawn there, the final tier from the native window) — the vector twin of the raster depth shading — and folds two more kinds into the one `depare` layer off the same `gdal_contour -p` pass and the OSM land + inland-water masks: green drying foreshore (the `(0, DRYING_CAP]` bucket seaward of the land line but kept inside mapped inland water, negative `drval1`; exact datum is the land sentinel and the merge's fill, never foreshore) and unknown-depth water (`nodata`, no `drval1`).
...
- **bundle** (`bundle.py`, `contour_run.py bundle`): concat single-zoom pmtiles into `planet.pmtiles` + one overlay archive per populated grid cell + `manifest.json` (which also carries `vector.max_zoom` = the covering's max child_z, the Worker's vector serving depth). The vector layers build SHARDED: a dense shallow run (z0 to the split) + one `--generate-variable-depth-tile-pyramid` run per populated stem-grid cell (`vector_shallow` / `vector_cell` / `vector_join` in build.smk; per-cell logs and benchmarks under those names), each cell filtered to its owned tile subtree and `pmtiles merge`d into the single sparse `vector.pmtiles` the Worker overzooms (no separate soundings/depare archives). The per-source coverage footprints bundle standalone into `coverage.pmtiles` (the `coverage` rule). Snakemake owns bundle freshness — a bundle rebuilds when a member tile changed.
...
- `vector.pmtiles` — MVT vector source with `contours`, `soundings`, and `depare` layers, `pmtiles merge`d from the shallow + per-cell shards above. It is a **sparse** pyramid: a tile stops subdividing once it carries its content at full detail, and the Worker synthesizes deeper tiles by overzooming the nearest baked ancestor (activated by `manifest.vector.max_zoom`). All zoom gating is per-feature `tippecanoe.minzoom` (contour tiers, depare's z6 floor, sounding pyramid levels), so a leaf freezes the right content. `depare` (z6+) carries depth-band partitions, drying foreshore (negative `drval1`), and unknown-depth water (`nodata`) in one layer. Direct-archive consumers (not via the Worker) need `pmtiles` JS ≥ 4.1 with `errorOnMissingTile: true`.
...
Isobath lines derive from the depth-area partition (`contour_run.py` reads `store/depare/<stem>.fgb`): a segment two bands of one ladder share is the contour at the level between them, so the drawn line and the band edge are the same polyline — lines can never cross each other and always match the fills. Seam continuity is inherited from depare's **buffer the DEM input, restrict the tile output** contract (deterministic merge → byte-identical overlap → lines meet at the clip). Contour tiles are written per tile in `store/contour` so clean tiles persist across runs.
...
- **Weekly cron + manual dispatch** (gc.yml) is the **only deletion path**: it deletes mosaic objects unreferenced by the serving `mosaic.gti` or by a live `build/<sha>/manifest.json`'s candidate, purges the retired store-hydrate prefixes, and keeps `build/<sha>/` (an R2 lifecycle rule handles that) out of scope. It shares the `r2-store` concurrency group so it never runs during a build or source refresh, refuses to delete if the serving set doesn't resolve cleanly, and defaults a manual run to a dry run. See [docs/build.md](docs/build.md#garbage-collection).
