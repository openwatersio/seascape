...
- `vector.pmtiles` — MVT vector source with `contours`, `soundings`, and `depare` layers, tile-joined from the shallow + per-cell shards above. It is a **sparse** pyramid: a tile stops subdividing once it carries its content at full detail, and the Worker synthesizes deeper tiles by overzooming the nearest baked ancestor (activated by `manifest.vector.max_zoom`). All zoom gating is per-feature `tippecanoe.minzoom` (contour tiers, depare's z6 floor, sounding pyramid levels), so a leaf freezes the right content. `depare` (z6+) carries depth-band partitions, drying foreshore (negative `drval1`), and unknown-depth water (`nodata`) in one layer. Direct-archive consumers (not via the Worker) need `pmtiles` JS ≥ 4.1 with `errorOnMissingTile: true`.
