...
Four stages — `source → aggregation → downsampling → bundle` — all writing under `pipelines/store/` (gitignored), one subdir per stage (`store/source`, `store/aggregation`, `store/pmtiles`, `store/contour`, `store/bundle`, …). Incrementality is content-hash keys (`keys.py`): each store artifact is **content-addressed** — its key (inputs ‖ the pipeline modules that produce it ‖ the resolved config it read ‖ the toolchain) rides in its filename (`<stem>-<key12>.<ext>`), so freshness is "the named file exists" and a stage rebuilds a name that isn't present. Code and config changes rebuild exactly their dependents, and `FORCE_REBUILD=1` is an escape hatch, not a correctness requirement (see [docs/build.md](docs/build.md#the-incremental-model)).

- **source** (`source_*.py`, per `sources/<id>/`): fetch → datum offset → normalize to a 4326 COG → `bounds.csv` → coverage polygon → tarball → a generated `catalog.json` (the machine-facing item the build reads for priority/max_zoom/land_clamp/offset + the recipe hash the tile keys consume; `metadata.json` stays the hand-edited input and fallback). Each source owns its recipe (`sources/<id>/Justfile`) composing the shared steps. Priority derives from `(maxzoom, id)` — GEBCO loses (smallest maxzoom), so regional sources win in overlap.
