...
- **Manual dispatch** (build.yml) runs the full build on **ephemeral Hetzner boxes** (the seamap `build-planet.yml` pattern; see [docs/build.md](docs/build.md)), in two phases over one DAG and one store volume — `build-phase.yml` is the reusable per-phase workflow (boot the box → attach the volume → one Snakemake invocation → always-destroy the box), called twice. **`build-wide`** takes the dispatched `server_type` and builds `prebundle`: the `cover` checkpoint materializes the covering from the volume's registrations, then the graph re-evaluates into every parallel job — mosaic merges → cartographic forks → terrain renders → per-cell vector archives → the mosaic index. **`build-tail`** then takes a fixed `ccx33` and builds `bundles publish_mosaic stage_build vector_selfcheck`: the serial, IO-bound finish (the shallow tippecanoe run, the pmtiles merges and concats, the R2 uploads), which would otherwise hold the big box at one core for hours. The second invocation re-plans the first phase's outputs as current, so it runs exactly the tail. `needs: build-wide` is the failure gate — a red wide phase skips the tail — and, because a called workflow finishes only once its own teardown has, it is also the volume handoff. A `bbox` slice or an explicit `targets` dispatch runs whole in the wide phase and skips the tail: neither has a tail worth a second boot. Incrementality is Snakemake engine provenance (a rule's inputs + params), so only stale artifacts rebuild; `publish_mosaic` content-addresses the finished mosaic tiles to R2 under hashed names and writes a **candidate** pointer, never touching the serving pointer. A build never contacts an upstream and runs no fetch — on a warm volume the covering is up-to-date, so cover's stage-1 producers stay dormant, and every byte (sources and the land/water masks alike) comes from R2 via `/vsicurl`. Build state and per-commit bundles persist in the public **data bucket** (`data.openwaters.io`) under `bathymetry/`. Dispatch-only on purpose. Manual runs accept an optional `bbox` (a self-contained regional slice that writes no planet pointer), a `force` flag (ignore freshness — an escape hatch for a suspect store), and a `server_type`.
...
`sources.yml`, `build.yml`, and `gc.yml` share one concurrency group (`r2-store`), so store mutations queue instead of interleaving. It is a run-level group, which covers every job of a called workflow — so both phases of a build hold it for the run's whole life.
