...
from shapely import box, from_wkt, get_parts, intersection, line_merge, union_all
...
RIVERS = {
    ...
    # the border river is name "Oder/Odra" (German/Polish) in Overture, not "Oder" alone
    "oder":  (["Oder/Odra", "Odra/Oder", "Odra", "Oder"], "13.9,52.0,14.85,53.15", None),  # Grenzoder (MNW ramp)
    # north edge = Bremen Weserwehr (km 362); beyond it is the tidal Unterweser (SKN reach), excluded
    "weser": (["Weser"], "8.4,51.35,9.75,53.08", None),             # Hauptweser (Ober+Mittel, MNW ramp)
...
def fetch_lines(names, bbox):
    ...
    # -spat returns whole features, so a way crossing the bbox edge drags in geometry outside the
    # intended reach (the Weser bleeds into the tidal Unterweser, the Oder into Poland). Clip to the
    # bbox so the corridor is exactly the reach — the bbox is the declared extent, and an out-of-reach
    # corridor would overwrite a neighbouring reference in the VRT (Weser MNW over Unterweser SKN).
    clipped = intersection(line_merge(union_all(lines)), box(float(w), float(s), float(e), float(n)))
    return line_merge(clipped)
