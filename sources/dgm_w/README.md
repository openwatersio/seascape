...
holds the direct URLs. The feed also carries 19 `Höhenlinien`
...
- **Inland reaches** (Rhein, Main, Mosel, Saar, Lahn, Grenzoder, Havel-Oder, upper Elbe above the
  ...
  **GlW**), the **upper Elbe** (km 0–586, 25 tiles, **MNW**), the **Grenzoder** (km 542–705, 8 tiles,
  **MNW**), and the **Ober-/Mittelweser** (Hauptweser km 0–366, 1 tile, **MNW** — the impounded
  Mittelweser's steps smoothed by the ramp) — and the impounded canalised rivers on **Stauziel** —
  the **Main** (km 0–102, 6 tiles), the **upper Rhein** (Basel→Iffezheim, km 164–334, 16 tiles), the
  **Mosel** (km 1.9–229.9, 1 tile), the **Saar** (km 0–94, 5 tiles), and the **Lahn** (km 80–136,
  3 tiles). The remaining inland tiles (Ems, the canals) stay **commented out** in `file_list.txt` —
  they publish no GlW/Stauziel, so they need an MNW proxy or a state-sourced low-water datum first.
...
subtracts a **low-water reference surface** — the local low-water datum expressed in NHN — so
...
Wasserstand, or **MNW** where GlW is un-published) for the free-flowing Rhein, Elbe, Grenzoder and
Ober-/Mittelweser, and **Stauziel** (per-pool retention level) for the impounded canalised rivers
(Main, upper Rhein, Mosel, Saar, Lahn). This is the scalar lake `--offset` (Bodensee) generalized to
a spatially-varying surface — the same `offset_surface` mechanism the CUDEM/VDatum correction uses.
...
`datum_surface_dgm_w` rule runs it) composes `store/datum/dgm_w_lowwater.tif`. Ten reaches, each
built as its own EPSG:4326 GeoTIFF, merged onto the finest grid for the single raster the datum
store expects:
   ...
   - **Free-flowing Rhein / Elbe / Grenzoder / Ober-/Mittelweser — `ramp_{rhein,elbe,oder,weser}.tif`**
     (`build_freeflowing`). Un-impounded rivers have no pools, so low water is a smooth downstream
     **ramp**, not a step. The Rhein below Iffezheim uses **GlW** (`rhein_glw.csv`); the Elbe (Czech
     border → Geesthacht km 586), the German/Polish border **Grenzoder** (km 542–705), and the
     **Hauptweser** (Hann.Münden → Bremen Weserwehr km 362) have no openly-published per-gauge GlW, so
     they use **MNW** (`<river>_mnw.csv`) — a few cm–dm below GlW, the conservative direction, and
     reproducible from PEGELONLINE. All harvested by `harvest_gauges.py`; the datum is interpolated
     along the gauge line by arc-length (low water drifts ~0.02 m/km, so the coarse gauge chord is fine
     ...
     upgrade to a Stauziel step reach (`build_impounded`) if a Mittelweser level table surfaces.
   ...
   - **Impounded Mosel / Saar / Lahn — `zs_mosel.tif` / `zs_saar.tif` / `zs_lahn.tif`** (`build_impounded`,
     ...
     Lahn 66→106 m.
The prep (driven by `offset_surface`/`clamp_positive` in `metadata.json`) reprojects the reference
onto each tile (bilinear, cross-CRS), subtracts it, and drops above-datum cells; normalize keeps
each tile's own CRS (`mixed_crs`) → COG.
...
for the Stauziel step — yielding water-only depths below their datum with land
...
**Remaining inland reaches (deferred).** Sourcing note: PEGELONLINE
(`pegelonline.wsv.de/webservices/rest-api/v2`) exposes each gauge's `gaugeZero` (PNP in NHN) and
its characteristic values — **GlW**, **ZS_I** (Stauziel) and **MNW** — so `harvest_gauges.py`
reads them straight from the API; where the gauges are too sparse for the pool structure (as on
the Main) a checked-in barrage-level table fills the gaps. Still commented out in `file_list.txt`:
the tidal **Ems** (needs the SKN path or MTnw — a different approach than the ramp) and the
**Dortmund-Ems-Kanal / Havel-Oder** canals (no PEGELONLINE gauges; need WSV Haltung levels). Each
reuses the same `build_reference.py` machinery — tracked separately.
