...
| CUDEM Hawaii ([1/9](cudem_pacific_hawaii/), [1/3](cudem_pacific_hawaii_third/)) | ~3.4/10 m | z13 / z12 | 8 main Hawaiian islands              | **MLLW** (+0.351 m scalar) |
| CUDEM Guam ([1/9](cudem_pacific_guam/), [1/3](cudem_pacific_guam_third/)) | ~3.4/10 m | z13 / z12 | Guam                                 | **MLLW** (+0.418 m scalar) |
| CUDEM CNMI ([1/9](cudem_pacific_cnmi/), [1/3](cudem_pacific_cnmi_third/)) | ~3.4/10 m | z13 / z12 | Saipan, Tinian, Rota                 | **MLLW** (+0.393 m scalar) |
| CUDEM Am. Samoa ([1/9](cudem_pacific_samoa/), [1/3](cudem_pacific_samoa_third/)) | ~3.4/10 m | z13 / z12 | Tutuila, Manu'a                      | ASVD02, uncorrected     |
...
CUDEM is ten sources, split on two axes. Resolution is the first: `max_zoom` is per-source, so every 1/9″ band and its 1/3″ counterpart are separate ids. NCEI dataset is the second, because both the horizontal frame and the vertical datum are properties of the dataset. CONUS, Alaska, Puerto Rico and the USVI are uniformly NAD83 (EPSG:4269) and all correctable from one VDatum grid, so they stay together as `cudem`/`cudem_third`'s 1,382 tiles. The Pacific territories are one source per island group: their horizontal frames genuinely differ (Hawaii is WGS 84, Guam's 1/9″ band is NAD83(MA11), the rest NAD83), VDatum publishes no grid anywhere out there, and each island group's separation from chart datum is its own number — which `datum_offset_m` can only express per source. Reading all 163 Pacific tile headers confirms each dataset is internally single-CRS, so none of them needs `mixed_crs` and all ten take the cheaper single-VRT aggregation path. Vertical datums play no part in the partitioning — every source's staged rasters lose the vertical half of a compound CRS at prep, so no warp can apply a geoid shift to the pixel values.
